...
public class RepositoryManager {
    private static final String SQLITE_SCHEMA_FILE_NAME = "szcore-schema-sqlite-create.sql";
     ...
     * @param sourceFile The source file to load (JSON or CSV).
     * @param dataSource The data source to use for loading the records.
     * @param recordMap The optional {@link Map} to populate with the
     *                  {@link SzRecordKey} keys mapped to {@link String}
     *                  record definition values for the records that 
     *                  were loaded.
     * @return <tt>true</tt> if successful, otherwise <tt>false</tt>
     */
    public static boolean loadFile(File                     repository,
                                   File                     sourceFile,
                                   String                   dataSource,
                                   Map<SzRecordKey,String>  recordMap) 
    {
        return loadFile(repository,
                false,
                sourceFile,
     ...
     * @param loadedCount The output parameter for the number successfully loaded.
     * @param failedCount The output parameter for the number that failed to load.
     * @param silent      <code>true</code> to silence trace output, otherwise
     *                    <code>false</code>.
     *
     * @return <tt>true</tt> if successful, otherwise <tt>false</tt>
     */
        ...
        try {
            for (JsonObject record = recordReader.readRecord(); (record != null); record = recordReader.readRecord()) {
                ...
                if (returnCode == 0) {
                    ...
                    if (recordMap != null) {
                        recordMap.put(SzRecordKey.of(dataSource, recordId),
                                      jsonRecord);
                    }
    ...
    /**
     * Gets the entity for a record identified by a data source code
     * and record ID.
     *
     * @param repository  The directory for the repository.
     * @param verbose     <tt>true</tt> for verbose API logging, otherwise
     *                    <tt>false</tt>
     * @param dataSource  The data source of the record to find the entity.
     * @param recordId    The record ID of the record to find the entity.
     *
     * @return The entity JSON describing the entity for the record,
     *         or <code>null</code> if there was a failure finding it.
     */
    public static String getEntity(File     repository,
                                   boolean  verbose,
                                   String   dataSource,
                                   String   recordId,
                                   long     flags)
    {
        return getEntity(repository, verbose, dataSource, recordId, flags, false);
    }
    ...
    public static String getEntity(File     repository,
                                   ...
                                   boolean  silent) 
    {
        return getEntity(repository, false, dataSource, recordId, flags, silent);
    }
                                   ...
                                   boolean  silent) 
    {
        ...
        int returnCode = ENGINE_API.getEntityByRecordID(
            dataSource, recordId, flags, sb);
        
        if (returnCode != 0) {
            System.out.println("FAILED to get entity for data source (" + dataSource
                    + ") with record ID (" + recordId + ")");
            System.out.println(
                    formatError("getEntityByRecordID()", ENGINE_API));
            return null;
        }

        String result = sb.toString();
        if (!silent) {
            System.out.println("Entity for record having data source (" + dataSource
